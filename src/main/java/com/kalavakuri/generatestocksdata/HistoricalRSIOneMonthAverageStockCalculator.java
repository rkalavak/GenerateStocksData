...
public class HistoricalRSIOneMonthAverageStockCalculator {
	...
	public static void main(String[] args) throws Exception {
		...
		for (StockVO stockVO : StocksDataUtil.getMoneyControlSymbols()) {
			...
			for (int j = 0; j < eachRow.length - 14; j++) {
				...
				double finalCalculation = 100 - (100 / (1 + (pointsGainAverage / pointsLostAverage)));
				RSIValues.add(finalCalculation);
			}
			int size = RSIValues.size();
			List<Double> subList = RSIValues.subList(size - 22, size);
			System.out.println(stockName + "\t"
					+ StocksDataUtil.format(subList.stream().mapToDouble(v -> v).average().getAsDouble()) + "\t"
