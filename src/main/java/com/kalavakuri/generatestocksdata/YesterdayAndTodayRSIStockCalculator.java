...
public class YesterdayAndTodayRSIStockCalculator {
	...
	public static void main(String[] args) throws Exception {
		...
		List<Double> calculations = new ArrayList<>();
		...
		for (StockVO stockVO : StocksDataUtil.getMoneyControlSymbols()) {
			...
			int startIndex = eachRow.length-16;

			for (int j = startIndex; j < eachRow.length - 14; j++) {
				int i = j;
				int size = i + 14;
				int k = i;
				pointsGain.clear();
				...
				calculations.add(finalCalculation);
			}
			System.out.println(stockName + "\t" + calculations.get(0) + "\t" + calculations.get(1));
		}
	}
}